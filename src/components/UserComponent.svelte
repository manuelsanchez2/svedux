<script lang="ts">
	import { useDispatch, useSelector } from '$lib/hooks.js';
	import { fetchUser, login, logout } from '../store/slices/userSlice.js';
...
<div class="user">
	...
	{#if user.value.isLoggedIn}
		<div>
			<p class="user__message">Welcome, {user.value.name || 'User'}!</p>
	...
	{:else}
		<div class="user__buttons">
			...
			<button
				class="user__button user__button--fetch"
				on:click={() => dispatch(fetchUser())}
				disabled={user.value.status === 'loading'}
			>
				{user.value.status === 'loading' ? 'Fetching...' : 'Fetch User'}
			</button>
		</div>
	{/if}
</div>
